    	...
    	if($('.product-grid').length)
    	{
    		$('.product-grid').isotope({
	            ...
	            getSortData: {
	            	...
	            	{
						var priceEle = $(itemElement).find('.input_product_price').text();
	        ...
	        filterButton.on('click', function()
	        {
	        	$('.product-grid').isotope({
		            filter: function()
		            {
						...
						var itemPrice = parseFloat($(this).find('.input_product_price').clone().children().remove().end().text()) ;
    ...
    function initPriceSlider()
    {
		$( "#slider-range" ).slider(
		{
			...
			slide: function( event, ui )
			{
				...
				//mình gắn 2 giá trị trên thanh slider vào 2 id
